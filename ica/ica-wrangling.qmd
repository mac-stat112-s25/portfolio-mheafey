...
# Filters to only keep data on counties in Delaware and Hawaii
```

```{r}
# Keep only data on counties where the Republicans got MORE THAN 93.97% of the vote in 2020
...
#creates a new column called "diff_20" that is the difference between the percentage of republican and democratic votes in 2020, then prints out the first 6 rows
```
...
#creates a new column called "repub_votes_20" that is calculated using the total votes and republican vote percentage
```
...
#creates a new column called "repub_win_20" that compares the republican and democratic vote percentages to determine if the republicans won in any given county in 2020.
```
...
I think this code will filter for only counties in the state of Wisconsin where democrats won in 2020, then arrange them in descending order
...
Filtering first gets rid of extraneous rows quickly
...
I think it will filter for counties in Delaware, create a new variable called repub_win_20 that shows if republicans won any given county in 2020, then select only the county_name, repub_pct_20, dem_pct_20, and repub_win_20 variables
...
mn_wi <- elections_small |>
  ...
  mutate("dem_change" = dem_pct_20 - dem_pct_16) |>
  select("dem_change", "state_name", "county_name", "dem_pct_20", "dem_pct_16") |>
  arrange(dem_change)
...
```
Minnesota on average had a higher positive change in the Democratic vote.
...
#returns the median of the repub_pct_20 variable
```
...
#creates a new variable that is the median of repub_pct_20
```
...
#creates two new variables, one that is the median of repub_pct_20, and one that is the total number of votes in every county
```
...
#calculates the median repub % and total votes for every state. if we didn't use group_by, it would only do it for all the data overall, not by state
```
...
elections_small |>
  group_by(state_name) |>
  summarize(total_dem = sum(total_votes_20*(dem_pct_20/100)), total_repub = sum(total_votes_20*(repub_pct_20/100)))
...
world_cup |>
  mutate(per_game = goals_scored/games) |>
  ggplot(aes(x = year, y = per_game)) +
  geom_line() +
  geom_point()
