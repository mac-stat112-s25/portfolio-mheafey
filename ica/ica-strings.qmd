...
courses_clean |>
  group_by(dept) |>
  summarize(avg_title = mean(str_length(name))) |>
  arrange(desc(avg_title)) |>
  head()
...
polisci <- courses_clean |>
  filter(dept == "POLI") |>
  mutate(foundations = str_detect(name, "Foundations")) |>
  mutate(America = str_detect(name, "US | American"))

polisci |>
  group_by(foundations) |>
  summarize(avg_enroll = mean(enroll))
#foundations courses have a higher enrollment on average

polisci |>
  mutate(time = fct_relevel(time, "8:00 - 9:30 am", "8:45 - 9:30 am" , "9:40 - 11:10 am", "10:50 - 11:50 am", "12:00 - 1:00 pm", "1:10 - 2:10 pm", "1:20 - 2:50 pm", "2:20 - 3:20 pm", "3:00 - 4:30 pm", "7:00 - 10:00 pm")) |>
  ggplot(aes(x = time)) +
...
courses_clean |>
  group_by(instructor) |>
  distinct(days, time, instructor, .keep_all = TRUE) |>
  summarize(instructor_classes = n()) |>
  ggplot(aes(x = instructor_classes)) +
  geom_bar()
