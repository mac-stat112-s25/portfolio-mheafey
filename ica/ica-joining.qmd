...
student_class_size <- grades |>
  ...
  summarize(student_median = median(enroll))
...
```{r}
ggplot(student_class_size, aes(x = student_median)) +
...
grades |>
  left_join(gpa_conversion) |>
  mutate(below_bplus = gp < 3.3) |>
  summarize(below_bplus = mean(below_bplus, na.rm = TRUE))
